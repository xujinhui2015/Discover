...
class ApplyForReturnOrderUnreviewService
{
    public function unreview(ApplyForReturnOrderModel $order): void
    {
        $order->loadMissing(['items', 'apply_for_order']);

        if ((int) $order->review_status !== ApplyForReturnOrderModel::REVIEW_STATUS_OK) {
            throw new RuntimeException('单据不是已审核状态，无法反审核');
        }

        if (! $order->apply_for_order) {
            throw new RuntimeException('未找到关联物料申请单，无法反审核');
        }

        $histories = StockHistoryModel::query()
            ->where('type', StockHistoryModel::RETURN_TO_WAREHOUSE_TYPE)
        ...
        DB::transaction(function () use ($order, $histories, $scale) {
            ...
            $order->review_status = ApplyForReturnOrderModel::REVIEW_STATUS_REREVIEW;
            $order->save();
        });
    }
}
