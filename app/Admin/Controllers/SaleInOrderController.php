...
class SaleInOrderController extends OrderController
{
    ...
    protected function grid()
    {
        return Grid::make(new SaleInOrder(['customer', 'user']), function (Grid $grid) {
                ...
                $grid->column('product_names', '物料名称')->display(function () {
                    $productNames = ProductModel::query()
                        ->whereIn('id', ProductSkuModel::query()
                            ->whereIn('id', SaleInItemModel::query()
            ...
            $grid->tools(BatchCreateSaleInOrder::make());
    ...
    protected function setForm(Form &$form): void
    {
        $form->row(function (Form\Row $row) {
            $row->width(6)->text('order_no', '单号')->default(build_order_no('TH'))->required()->readOnly();
        ...
        $form->row(function (Form\Row $row) use ($with_order) {
            ...
            if ($order && $order->review_status === $review_statu_ok) {
                $row->width(6)->select('status', '单据状态')->options(SaleInOrderModel::STATUS)->default($this->oredr_model::STATUS_SEND)->required();
                $row->width(6)->select('with_id', '相关单据')->options(SaleOutOrderModel::query()->pluck('order_no', 'id'))->disable();
            } else {
