...
use App\Services\ApplyForReturnOrderUnreviewService;
use Dcat\Admin\Grid\Tools\AbstractTool;
use Illuminate\Http\Request;
use Illuminate\Support\Str;

class ApplyForReturnOrderUnreview extends AbstractTool
{
    protected $title = '反审核';
    ...
    {
        $id = $request->input('id');
        $order = ApplyForReturnOrderModel::query()->find($id);

        if (! $order) {
            return $this->response()->error('未找到物料返仓单');
        }

        try {
            app(ApplyForReturnOrderUnreviewService::class)->unreview($order);
