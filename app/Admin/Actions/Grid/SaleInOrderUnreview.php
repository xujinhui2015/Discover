...
use App\Services\SaleInOrderUnreviewService;
use Dcat\Admin\Grid\Tools\AbstractTool;
use Illuminate\Http\Request;
use Illuminate\Support\Str;

class SaleInOrderUnreview extends AbstractTool
{
    protected $title = '反审核';
    ...
    {
        $id = $request->input('id');
        $order = SaleInOrderModel::query()->find($id);

        if (! $order) {
            return $this->response()->error('未找到客户退货单');
        }

        try {
            app(SaleInOrderUnreviewService::class)->unreview($order);
