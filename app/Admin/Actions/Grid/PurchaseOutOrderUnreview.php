...
use App\Services\PurchaseOutOrderUnreviewService;
use Dcat\Admin\Grid\Tools\AbstractTool;
use Illuminate\Http\Request;
use Illuminate\Support\Str;

class PurchaseOutOrderUnreview extends AbstractTool
{
    protected $title = '反审核';
    ...
    {
        $id = $request->input('id');
        $order = PurchaseOutOrderModel::query()->find($id);

        if (! $order) {
            return $this->response()->error('未找到采购退货单');
        }

        try {
            app(PurchaseOutOrderUnreviewService::class)->unreview($order);
