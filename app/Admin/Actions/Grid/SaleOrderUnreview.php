...
use App\Services\SaleOrderUnreviewService;
use Dcat\Admin\Grid\Tools\AbstractTool;
use Illuminate\Http\Request;
use Illuminate\Support\Str;

class SaleOrderUnreview extends AbstractTool
{
    protected $title = '反审核';
    ...
    {
        $id = $request->input('id');
        $order = SaleOrderModel::query()->find($id);

        if (! $order) {
            return $this->response()->error('未找到客户要货单');
        }

        try {
            app(SaleOrderUnreviewService::class)->unreview($order);
