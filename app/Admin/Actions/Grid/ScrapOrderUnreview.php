...
use App\Services\ScrapOrderUnreviewService;
use Dcat\Admin\Grid\Tools\AbstractTool;
use Illuminate\Http\Request;
use Illuminate\Support\Str;

class ScrapOrderUnreview extends AbstractTool
{
    protected $title = '反审核';
    ...
    {
        $id = $request->input('id');
        $order = ScrapOrderModel::query()->find($id);

        if (! $order) {
            return $this->response()->error('未找到物料报废单');
        }

        try {
            app(ScrapOrderUnreviewService::class)->unreview($order);
