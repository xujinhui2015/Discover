...
use App\Services\ApplyForOrderUnreviewService;
use Dcat\Admin\Grid\Tools\AbstractTool;
use Illuminate\Http\Request;
use Illuminate\Support\Str;

class ApplyForOrderUnreview extends AbstractTool
{
    protected $title = '反审核';
    ...
    {
        $id = $request->input('id');
        $order = ApplyForOrderModel::query()->find($id);

        if (! $order) {
            return $this->response()->error('未找到物料申请单');
        }

        try {
            app(ApplyForOrderUnreviewService::class)->unreview($order);
