...
use App\Services\SaleOutOrderUnreviewService;
use Dcat\Admin\Grid\Tools\AbstractTool;
use Illuminate\Http\Request;
use Illuminate\Support\Str;

class SaleOutOrderUnreview extends AbstractTool
{
    protected $title = '反审核';
    ...
    {
        $id = $request->input('id');
        $order = SaleOutOrderModel::query()->find($id);

        if (! $order) {
            return $this->response()->error('未找到客户出货单');
        }

        try {
            app(SaleOutOrderUnreviewService::class)->unreview($order);
