...
### 3.6 物料导入
- 进入「物料档案」列表，点击右上角的【导入物料】按钮即可打开导入页面；
- 按模板的表头填充数据，支持 `xlsx`/`xls`/`csv`，属性需要采用 `属性=值1,值2;属性2=值3,值4` 的格式；
- 属性与属性值需提前在「物料属性」模块维护，导入时会自动匹配并生成所有 SKU 组合；
- 相同的物料编号会被视为更新，其他数据为空则跳过；
- 页面内提供模板下载链接以及导入结果提示，方便快速排查错误。
